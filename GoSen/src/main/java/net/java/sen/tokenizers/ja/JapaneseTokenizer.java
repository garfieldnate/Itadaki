...
public class JapaneseTokenizer extends Tokenizer {
    ...
    private int getCharClass(char c) {
        if (c <= 0x7F) {
            return Character.isWhitespace(c) ? SPACE : Character.getType(Character.toLowerCase(c));
        } else if (c >= 0x3040 && c <= 0x309F) {
            return HIRAGANA;
        } else if ((c >= 0x30A0 && c <= 0x30FF && c != 0x30FB) || (c >= 0x31F0 && c <= 0x31FF) || (c >= 0xFF66
                                                                                                   && c <= 0xFF9F)) {
            return KATAKANA;
            ...
            if (ub == LATIN_1_SUPPLEMENT || ub == LATIN_EXTENDED_ADDITIONAL || ub == LATIN_EXTENDED_A
                || ub == LATIN_EXTENDED_B || ub == LATIN_EXTENDED_C || ub == LATIN_EXTENDED_D) {
                // Latin Letters
                return Character.getType(Character.toLowerCase(c));
            } else if (ub == GREEK || ub == GREEK_EXTENDED) {
                // Greek
                return Character.getType(Character.toLowerCase(c));
            } else if (ub == CYRILLIC || ub == CYRILLIC_EXTENDED_A || ub == CYRILLIC_EXTENDED_B
                       || ub == CYRILLIC_SUPPLEMENTARY) {
                // Cyrillic
                return Character.getType(Character.toLowerCase(c));
            } else {
                // Symbols
    ...
    public Node lookup(SentenceIterator iterator, char[] surface) {
        ...
        Node unknownNode = getUnknownNode(iterator.origin(), unknownTokenLength, skipped + unknownTokenLength);

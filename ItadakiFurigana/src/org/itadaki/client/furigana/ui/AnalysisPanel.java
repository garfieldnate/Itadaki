...
public class AnalysisPanel extends JPanel implements MouseListener, MouseMotionListener {
	...
	public void setReadingResult (ReadingResult readingResult, String text) {
		...
		int firstVisibleTokenIndex = this.readingResult.getVisibleTokens().nextSetBit(0);
		if (firstVisibleTokenIndex != -1) {
			Token token = this.readingResult.getTokens().get(firstVisibleTokenIndex);
			Selection selection = new Selection(firstVisibleTokenIndex, 0, firstVisibleTokenIndex, token.getLength() - 1);
	...
	/**
	 * Update the tokens and selection to be displayed
	 * 
	 * @param readingResult The reading processor result to be displayed 
	 * @param selection The selection to be displayed
	 */
	public void updateReadingResultAndSelection (ReadingResult readingResult, Selection selection) {
